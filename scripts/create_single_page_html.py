...
url_replacements = {"/": "index.md", "/about_this_book.html": "about_this_book.md"}


def _replace_url(url):
    return url.strip("/") if url not in url_replacements else url_replacements[url]
...
def render_math(output_filename=OUTPUT_FILENAME):
    ...
    with open(output_filename) as book, open(TEMP_FILENAME, "w") as temp:
        check_call(
            ["node_modules/mathjax-node-page/bin/mjpage", "--dollars", "--format", '"TeX"'], stdin=book, stdout=temp
        )
...
def fix_image_paths(output_filename=OUTPUT_FILENAME):
    ...
    can locate them.
    """
    exp = r"s#/notebooks-images#http://localhost:4000/notebooks-images#g"
    check_call(f'sed -i "" {exp} {output_filename}', shell=True)
